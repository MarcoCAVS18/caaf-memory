...
function ExitConfirmModal({ onConfirm, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 z-[9997] flex items-center justify-center p-6">
      ...
      <Card
        elevation="high"
        className="relative z-10 w-full max-w-sm p-7 flex flex-col gap-6"
      >
...
export default function GamePage() {
  ...
  // If the player is resuming, load the saved state; otherwise start fresh
  const savedState = state?.resume ? getSavedGame() : null
  ...
  // When starting a fresh game, wipe any previously saved game
  useEffect(() => {
    if (!state?.resume) clearSavedGame()
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  // Navigate to results when game ends naturally (clear saved game first)
  useEffect(() => {
  ...
  // Block in-app navigation while the game is in progress
  const blocker = useBlocker(phase === 'playing')

  function handleConfirmExit() {
    saveGame({ difficulty, cards: rawCards, elapsed, failedAttempts, matchedPairs: rawMatchedPairs })
    blocker.proceed()
  }
